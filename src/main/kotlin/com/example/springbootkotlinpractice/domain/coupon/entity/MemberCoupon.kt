...
class MemberCoupon(
    ...
    @Column(
        name = "coupon_id", nullable = false, updatable = false,
        comment = "쿠폰 템플릿 ID (coupons.id)",
    )
    val couponId: Long,

    @Enumerated(EnumType.STRING)
    @Column(name = "status", nullable = false, length = 20, comment = "쿠폰 상태 (UNUSED/USED/EXPIRED)")
    var status: MemberCouponStatus = MemberCouponStatus.UNUSED,

    @Column(name = "order_id", nullable = true, comment = "사용된 주문 ID (orders.id, 미사용 시 NULL)")
    var orderId: Long? = null,

    @Column(name = "issued_at", nullable = false, updatable = false, comment = "발급 일시")
    val issuedAt: LocalDateTime = LocalDateTime.now(),

    @Column(name = "used_at", nullable = true, comment = "사용 일시 (미사용 시 NULL)")
    var usedAt: LocalDateTime? = null,

    @Column(
        name = "expired_at", nullable = false, updatable = false,
