...
(
    ...
    totp_secret      VARCHAR(255) NULL COMMENT 'TOTP 시크릿 키 (AES 암호화 저장, 활성화 시에만 존재)',
    totp_enabled     BOOLEAN      NOT NULL DEFAULT FALSE COMMENT 'TOTP 2단계 인증 활성화 여부',
...
(
    id                 BIGINT AUTO_INCREMENT
        PRIMARY KEY,
    name               VARCHAR(100)  NOT NULL COMMENT '쿠폰 명',
    discount_type      VARCHAR(20)   NOT NULL COMMENT '할인 방식 (FIXED/PERCENTAGE)',
    discount_value     INT           NOT NULL COMMENT '할인액(FIXED) 또는 할인율%(PERCENTAGE)',
...
(
    id               BIGINT AUTO_INCREMENT
        PRIMARY KEY,
    member_id        BIGINT                        NOT NULL COMMENT '쿠폰을 보유한 회원 ID (members.id)',
    coupon_id        BIGINT                        NOT NULL COMMENT '쿠폰 템플릿 ID (coupons.id)',
    status           VARCHAR(20) DEFAULT 'UNUSED'  NOT NULL COMMENT '쿠폰 상태 (UNUSED/USED/EXPIRED)',
    order_id         BIGINT                        NULL COMMENT '사용된 주문 ID (orders.id, 미사용 시 NULL)',
...
CREATE INDEX idx_member_coupons_01
...
(
    id               BIGINT AUTO_INCREMENT
        PRIMARY KEY,
    member_id        BIGINT                       NOT NULL COMMENT '포인트를 보유한 회원 ID (members.id)',
    point_id         BIGINT                       NOT NULL COMMENT '포인트 템플릿 ID (points.id)',
    amount           INT                          NOT NULL COMMENT '원 적립 금액',
    remaining_amount INT                          NOT NULL COMMENT '남은 사용 가능 금액',
    status           VARCHAR(20) DEFAULT 'ACTIVE' NOT NULL COMMENT '포인트 상태 (ACTIVE/EXHAUSTED/EXPIRED)',
    issued_at        DATETIME(6)                  NOT NULL COMMENT '적립 일시',
    expired_at       DATETIME(6)                  NULL COMMENT '사용 가능 마감 일시 (NULL이면 무제한)',
    created_datetime DATETIME(6)                  NOT NULL,
    updated_datetime DATETIME(6)                  NOT NULL
)
    COMMENT '회원별 포인트 적립 내역 (잔액 관리)';

CREATE INDEX idx_member_points_01
    ON member_points (member_id, status, expired_at);

CREATE TABLE member_point_usages
(
    id               BIGINT AUTO_INCREMENT
        PRIMARY KEY,
    order_id         BIGINT      NOT NULL COMMENT '포인트를 사용한 주문 ID (orders.id)',
    member_point_id  BIGINT      NOT NULL COMMENT '차감된 포인트 적립건 ID (member_points.id)',
    used_amount      INT         NOT NULL COMMENT '이 적립건에서 사용한 금액',
    created_datetime DATETIME(6) NOT NULL,
    updated_datetime DATETIME(6) NOT NULL
)
    COMMENT '포인트 사용 내역 (주문 취소/반품 시 원복 기준)';

CREATE INDEX idx_member_point_usages_01
    ON member_point_usages (order_id);
...
(
    ...
    coupon_discount_price INT       DEFAULT 0                 NOT NULL COMMENT '쿠폰으로 할인된 금액 (미사용 시 0)',
    point_discount_price  INT       DEFAULT 0                 NOT NULL COMMENT '포인트로 할인된 금액 (미사용 시 0)',
...
-- 아래는 이미 떠 있는 real DB(local/dev)에 쿠폰/포인트 기능을 반영하기 위한 1회용 안내문이다.
-- 반영 후 바로 삭제할 것 (히스토리 기록이 아님).
-- CREATE TABLE coupons
-- (
...
--     updated_datetime DATETIME(6)                   NOT NULL
-- ) COMMENT '회원별 쿠폰 발급/보유 내역';
-- CREATE INDEX idx_member_coupons_01 ON member_coupons (member_id, status);
--
-- CREATE TABLE member_points
-- (
--     id               BIGINT AUTO_INCREMENT PRIMARY KEY,
...
--     updated_datetime DATETIME(6)                  NOT NULL
-- ) COMMENT '회원별 포인트 적립 내역 (잔액 관리)';
-- CREATE INDEX idx_member_points_01 ON member_points (member_id, status, expired_at);
--
-- CREATE TABLE member_point_usages
-- (
--     id               BIGINT AUTO_INCREMENT PRIMARY KEY,
...
--     updated_datetime DATETIME(6) NOT NULL
-- ) COMMENT '포인트 사용 내역 (주문 취소/반품 시 원복 기준)';
-- CREATE INDEX idx_member_point_usages_01 ON member_point_usages (order_id);
--
-- ALTER TABLE orders ADD COLUMN coupon_discount_price INT NOT NULL DEFAULT 0 COMMENT '쿠폰으로 할인된 금액 (미사용 시 0)' AFTER delivery_price;
-- ALTER TABLE orders ADD COLUMN point_discount_price INT NOT NULL DEFAULT 0 COMMENT '포인트로 할인된 금액 (미사용 시 0)' AFTER coupon_discount_price;
