...
- **엔티티 생성은 companion object의 `of()` / `ofOAuth()` 팩토리 메서드**를 통해서만 한다 (public
  생성자를 직접 호출하지 않음). 예: `Member.of(...)`, `OrderInfo.of(...)`, `PayInfo.of(...)`.
...
- **FK 성격의 컬럼/필드명은 참조 테이블명 + `_id`** 컨벤션을 따른다 (`delivery_info_id`,
  `order_info_id`, `product_info_id`). `order_id`/`product_id`처럼 축약하지 않는다.
- **Spring Data 파생 쿼리 메서드명은 엔티티의 Kotlin 프로퍼티명과 정확히 일치해야 한다** (DB 컬럼명이
  아니라). 엔티티에 `@ManyToOne val orderInfo: OrderInfo`가 있으면 `findByOrderId`가 아니라
  `findByOrderInfoId`여야 한다 — 틀리면 `PropertyReferenceException`으로 Spring 컨텍스트 자체가
  기동 실패하고, 컨텍스트를 공유하는 모든 `@SpringBootTest` 테스트가 도미노로 실패한다.
...
- **재고 차감은 조건부 UPDATE로 원자적으로 처리** (`ProductInfoRepository.decreaseStock`,
  `WHERE stock_count >= :count`) — 동시 주문에 의한 초과 판매 방지. 영향받은 row가 0이면
  `NOT_ENOUGH_STOCK`.
- **가격 스냅샷**: `OrderDetailInfo.price`(상품 가격), `OrderInfo.deliveryPrice`(배송비)는 주문
  시점 값을 스냅샷으로 저장한다. `product_info.price`/`delivery_info.price`는 이후 관리자가 바꿀 수
  있으므로, 주문 조회/결제 금액 검증 시 **절대 라이브 조회값을 다시 계산에 쓰지 않는다** — 반드시
  `order_info`/`order_detail_info`에 저장된 스냅샷을 사용한다.
- **주문 상태 이력**: `OrderStatusHistory`가 `OrderInfo.status`가 바뀔 때마다(생성 시
